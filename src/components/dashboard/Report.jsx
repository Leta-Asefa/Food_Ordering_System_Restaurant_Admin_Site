...
const Report = () => {
    ...
    useEffect(() => {
        async function fetchData() {
            const start=formatTheDate(startDate,'s')
            const end=formatTheDate(endDate,'e')
    ...
    const topLocation = locationArray[0];
    ...
    return (
        <div className="container mx-auto p-4">
            ...
            <div className='grid grid-cols-2 gap-5'>
                ...
                {/* Summary Report */}
                <div className="bg-gray-50 shadow-lg rounded-lg p-6 mb-6">
                    <h2 className="text-xl font-bold mb-4">Summary</h2>
                    <p className="text-lg">💰 Total Income: <strong>${totalIncome.toFixed(2)}</strong></p>
                    <p className="text-lg">📦 Total Orders: <strong>{orderCount}</strong></p>
                    <p className="text-lg">🏆 Most Sold Item: <strong>{mostSoldItem ? `${mostSoldItem[0]} (${mostSoldItem[1]} units)` : 'N/A'}</strong></p>
                    <p className="text-lg">🚩 Least Sold Item: <strong>{leastSoldItem ? `${leastSoldItem[0]} (${leastSoldItem[1]} units)` : 'N/A'}</strong></p>
                   <p className="text-lg">📍 Top Location: <strong>{topLocation ? `${topLocation[0]} (${topLocation[1]} orders)` : 'N/A'}</strong></p>
                ...
                <div className="bg-gray-50 shadow-lg rounded-lg p-6 mb-6">
                    ...
                    {Object.entries(paymentMethods).map(([method, count]) => (
                        <p key={method} className="flex flex-row gap-3 justify-start text-lg">
